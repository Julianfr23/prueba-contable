...
from app.domain.models import Cuenta, Empresa, Tercero
...
async def listar_cuentas(
    ...
    result = await session.execute(
        select(Cuenta).where(Cuenta.empresa_id == empresa_id)
    )
    return result.scalars().all()


@router.post("/empresas/{empresa_id}/terceros", response_model=TerceroOut)
...
@router.get(
    "/empresas/{empresa_id}/terceros",
    response_model=list[TerceroOut],
)
...
@router.post(
    "/empresas/{empresa_id}/periodos/{anio}/{mes}/cerrar",
    response_model=PeriodoOut,
)
