...
listarTerceros: (empresaId: string) =>
...
listarComprobantes: (empresaId: string) =>
request<Comprobante[]>(
`/comprobantes?empresa_id=${encodeURIComponent(empresaId)}`
),
...
request<Comprobante>("/comprobantes", {
method: "POST",
body: JSON.stringify(data),
}),

contabilizar: (comprobanteId: string) =>
request<Comprobante>(`/comprobantes/${comprobanteId}/contabilizar`, {
method: "POST",
}),

revertir: (comprobanteId: string, motivo: string) =>
request<Comprobante>(`/comprobantes/${comprobanteId}/revertir`, {
method: "POST",
...
request<ExogenaHistorial[]>(
`/exogena/historial?empresa_id=${encodeURIComponent(empresaId)}`
),

exogenaGenerarUrl: () => `${API_URL}/exogena/generar`,

exogenaArchivoUrl: (id: string) =>
`${API_URL}/exogena/historial/${id}/archivo`,
};
